...
namespace Enemies.Golem.States
{
    public class GolemChaseState : GolemBaseState
    {
        ...
        public override void OnEnter()
        {
            ...
            GolemStateMachine.Animator.Play("Walk-BlendTree"); // Play the running animation
        }
        ...
        {
            if (!GolemStateMachine.HasLineOfSight) return;
            ...
            ExecuteSetDestinationCommand(GolemStateMachine.PlayerCollider.transform.position);

            // Update movement-related animation parameters
            UpdateAnimationParameters(GolemStateMachine.EnemyNavMeshAgent.velocity.normalized);
        }
        ...
        {
            var distanceToPlayer = (GolemStateMachine.Rigidbody.transform.position -
            ...
            else if (distanceToPlayer >= GolemStateMachine.EnemyProperties.ChaseRadius)
            {
                ...
                GolemStateMachine.SwitchState(GolemStateMachine.GolemIdleState);
            }
        }
